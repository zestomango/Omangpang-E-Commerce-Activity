...
const ViewGame = () => {
  const { id } = useParams();
  const { getGameById, addToCart } = useGameContext();
  ...
  const gameData = {
    ...
    description:
      passedGame?.description ||
      "Don the mask and join the Phantom Thieves of Hearts as they stage grand heists, infiltrate the minds of the corrupt, and make them change their ways! Prepare for the award-winning RPG experience in this definitive edition of Persona 5 Royal, featuring a wealth of downloadable content included!",
  ...
  const handleAddToCart = () => {
    addToCart({ ...passedGame, ...gameData }, quantity);
      ...
      >
          ...
          >
            {gameData.thumbnails.map((thumb, idx) => {
              const thumbUrl =
                typeof thumb === "string"
                  ? thumb
                  : thumb.thumbnail || thumb.url;
              const isVideo =
                typeof thumb === "object" && thumb.type === "video";
              ...
              return (
                <div
                  ...
                  onMouseOver={(e) => {
                    if (!isActive)
                      e.currentTarget.firstChild.style.borderColor = "#8f98a0";
                  }}
                  onMouseOut={(e) => {
                    if (!isActive)
                      e.currentTarget.firstChild.style.borderColor =
                        "transparent";
                  }}
                >
                  <img
        ...
        >
          ...
          >
            <span>
              DEVELOPER:{" "}
              <span style={{ color: "#66c0f4" }}>{gameData.developer}</span>
            </span>
            <span>|</span>
            <span>
              PUBLISHER:{" "}
              <span style={{ color: "#66c0f4" }}>{gameData.publisher}</span>
            </span>
          </div>
          ...
          {showConfirmation && (
            ...
            >
              {gameData.title} has been successfully added to your cart.
            </div>
          )}
        </div>
      </div>
